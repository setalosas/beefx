...
const {div$, leaf$, set$, setClass$, q$$, haltEvent, onReadyStateComplete} = DOMplusUltra
const {round} = Math
  ...
  const initMixerCommandHandlers = _ => {
    ...
    ui.toggleGrab = ui.toggleCmd(mixer, ui.grabCmd$, 'isGrabOn', on => {
      if (on) {
        ...
        const grabs = [...q$$('a#thumbnail > .bfx-grab-frame')]
    ...
    ui.toggleSync = ui.toggleCmd(mixer, ui.syncCmd$, 'isSyncOn', on => {
      setClass$(ui.syncFrame$, !on, 'off') //+ atnevezni syncframe
      //setClass$(ui.videoStrip$, !on, 'off')
      //setClass$(ui.mixermenu$, !on, 'off')
    })
  }
